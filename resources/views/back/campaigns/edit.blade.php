        ...
        <div class="card-body">
            <div class="form-floating mb-3">
                <input type="text" class="form-control" name="title" id="title" placeholder="Title" required
                       maxlength="255" value="{{ $campaign->title }}"/>
            ...
            <div class="form-floating mb-3">
                <input type="text" class="form-control" name="subtitle" id="subtitle" placeholder="Subtitle" required
                       maxlength="255" value="{{ $campaign->subtitle }}"/>
            ...
            <div class="mb-3">
				...
				<textarea class="form-control" name="text" id="text" placeholder="Text" required
                >{{ $campaign->text }}</textarea>
            </div>
            @error('text')
            <div class="alert alert-danger">{{ $message }}</div>
            @enderror
            <div class="mb-3">
                ...
                <input type="file" name="image" id="image" class="dropify" data-show-remove="false"
                       @if($campaign->image) data-default-file="{{ asset('storage/'. $campaign->image) }}" @endif/>
                @if($campaign->image)
                    <a href="{{ route('admin.campaigns.deleteImage', $campaign->id) }}" class="btn btn-danger mt-3">
                        Delete Image
                    </a>
                @endif
            </div>
